...
@test("ETF flows (Farside → CoinGlass v16)", None)
def test_farside():
    """v16 — flux ETF avec fallback : Farside d'abord, puis CoinGlass keyless."""
    from src.data_sources import etf_flows as _ef
    res = _ef.get_etf_flows()
    if res.get("available"):
        _src = (res.get("btc") or res.get("eth") or {}).get("source", "?")
        _btc = (res.get("btc") or {}).get("total_flow_musd")
        report("ETF flows (Farside → CoinGlass v16)", "ok",
               f"Source : {_src} · flux BTC du jour : "
               f"{_btc if _btc is not None else 'n/d'} M$")
    else:
        report("ETF flows (Farside → CoinGlass v16)", "warn",
               "Farside (403) ET CoinGlass indisponibles — angle mort ETF "
               "signalé dans le mail, AUCUNE action requise")
...
@test("Polymarket étendu (barres Fed + marchés majeurs)", None)
def test_polymarket_extended():
    ...
    if fb.get("dominant"):
        report("Polymarket étendu (barres Fed + marchés majeurs)", "ok",
               f"Dominant : {fb['dominant']} {fb.get('dominant_pct')}% · "
               f"{len(extra)} autre(s) marché(s) majeur(s)")
    elif res.get("available"):
        report("Polymarket étendu (barres Fed + marchés majeurs)", "warn",
               "Marchés Fed reçus mais agrégation barres incomplète "
               "(probas hors patterns) — fallback % baisse actif, "
               "AUCUNE action requise")
    else:
        report("Polymarket étendu (barres Fed + marchés majeurs)", "ko",
               "Aucun marché reçu")


@test("Calendrier macro consolidé (FRED+Boursorama+BCE/FOMC/BoJ)", None)
def test_macro_calendar_consolidated():
    ...
    if evts:
        nxt = evts[0]
        report("Calendrier macro consolidé (FRED+Boursorama+BCE/FOMC/BoJ)", "ok",
               f"{len(evts)} événement(s) sur 10j · prochain : "
               f"{nxt.get('label')} ({nxt.get('when')}) · "
               f"sources : {', '.join(res.get('sources_used') or []) or 'repli'}")
    else:
        report("Calendrier macro consolidé (FRED+Boursorama+BCE/FOMC/BoJ)", "ko",
...
def main() -> int:
    print("\n═══ Diagnostic Crypto Analyst Pro · v28 ═══\n")
    sections = [
