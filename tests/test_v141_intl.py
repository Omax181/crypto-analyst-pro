...
def test_10y_not_divided_when_already_in_percent(monkeypatch):
    # v18 (M-A24) — RÉGRESSION : Yahoo renvoie parfois ^TNX DÉJÀ en % (4.487),
    # pas en ×10. La conversion aveugle ÷10 donnait 0.4487 → masqué (< 0.5).
    # La détection de magnitude doit laisser 4.487 intact.
    from src.data_sources import market_prices as mp

    def fake_get_json(url, headers=None, params=None, **kw):
        if "%5ETNX" in url or "^TNX" in url:
            return _chart_payload(4.487, prev=4.46)  # déjà en %
        return _chart_payload(200.0, prev=198.0)
    ...
    quotes = mp.get_macro_quotes()
    deltas = mp.get_macro_deltas()
    # Valeur déjà en % (< 20) → conservée telle quelle, PAS divisée.
    assert quotes["us_10y"] == pytest.approx(4.487)
    assert deltas["us_10y"] == pytest.approx(0.027, abs=0.001)
    # Et surtout : dans la plage valide [0.5, 12], donc plus jamais masquée.
    assert 0.5 <= quotes["us_10y"] <= 12.0
...
def test_onchain_metrics_falls_back_to_mirror(monkeypatch):
    ...
    # v21 — neutralise la surcouche fraîcheur bitcoin-data.com (sinon appel réseau
    # + écrasement du MVRV BTC) : ce test cible spécifiquement le repli miroir.
...
def test_unlocks_defillama_parsing_window_and_mapping(monkeypatch):
    ...
    # v21 — DefiLlama emissions est payant (402) : le module est désactivé par
    # défaut. On réactive explicitement pour tester la logique de parsing.
    monkeypatch.setenv("DEFILLAMA_PAID", "1")
...
def test_unlocks_graceful_on_unexpected_schema(monkeypatch):
    ...
    monkeypatch.setenv("DEFILLAMA_PAID", "1")  # v21 — teste le parsing, pas le gate 402
...
def test_render_morning_intl_group_and_equity_cell():
    ...
    # v18 (M-B9) : BCE + BoJ sont désormais inlinés en une ligne « Taux directeurs »
...
def test_render_morning_without_intl_hides_group():
    ...
    # v18 (M-B9) : EUR/USD et USD/JPY sont désormais dans cette section aussi,
    # il faut donc les retirer pour la masquer entièrement.
