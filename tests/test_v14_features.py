...
- _build_evening_reco_bilan (dédup par actif + 3 statuts)
...
# --------------------------------------------------------------------------- #
# fmt_money — format français avec devise
# --------------------------------------------------------------------------- #
def test_fmt_money_french_format():
...
def test_fmt_money_edge_cases():
    ...
    # micro-prix : 6 chiffres significatifs, pas de notation scientifique
...
def test_reco_bilan_dedup_one_row_per_asset():
    ...
    assert len(bilan) == 4  # BTC dédupliqué (5 thèses -> 4 lignes)
...
def test_reco_bilan_statuses():
    ...
    assert bilan["LINK"]["status"] == "no_trigger"     # SURVEILLER, pas d'entrée
...
def test_morning_plan_fmt_money():
    from src.reporting.email_html import render
    html = render(_morning_payload(), "morning")
    assert "63.180,00 $" in html   # entrée
    assert "68.214,17 $" in html   # TP
    assert "60.500,00 $" in html   # SL
...
def test_merge_python_facts_sorts_string_confidence():
    ...
    payload = {
        "thesis_of_the_day": [
            {"asset": "A", "action_type": "neutral", "confidence": "65%"},
            {"asset": "B", "action_type": "bullish", "confidence": "72%"},
...
def test_render_morning_md_and_outlook_tables():
    ...
    assert 'text-align:right;">+7.8%' in html
    assert 'text-align:right;">−6.2%' in html
...
def test_render_evening_micro_price_not_masked():
    ...
    html = render(
        {"reco_bilan": [{"asset": "CKB", "action": "SURVEILLER",
                         "entry": 0.00129, "current": 0.00131,
                         "delta_pct": 1.6, "status": "validated"}]},
        "evening",
    )
    i = html.find("CKB")
    row = html[i:i + 600]
    assert "→" in row and "—" not in row.split("→")[0][-40:]
