...
def _uploads_ok(channel_id: str, key: str) -> bool:
    """Probe 1 unité : la playlist uploads (UU…) de la chaîne répond-elle ?

    Rejette les chaînes auto-générées (« - Topic ») et les ID invalides qui
    provoquaient des 404 récurrents sur playlistItems (v21, Logs#6).
    """
    ...
    data = get_json(
        ...
        params={"part": "id", "playlistId": uploads_id, "maxResults": 1, "key": key},
    )
    # get_json renvoie None sur 404 ; un dict (même items=[]) = playlist valide.
...
def _resolve_channel_id(name: str, key: str) -> str | None:
    ...
    def _do_resolve() -> str | None:
        ...
        #    v21 (Logs#6) — la recherche peut renvoyer une chaîne auto-générée
        #    « … - Topic » SANS playlist uploads (UU…), d'où le 404 récurrent sur
        #    playlistItems. On prend plusieurs candidats et on retient le PREMIER
        #    dont la playlist uploads répond réellement (probe 1 unité).
        data = get_json(
            f"{_BASE}/search",
            params={
                "part": "snippet",
        ...
        for it in items:
            ...
            if not cid or not cid.startswith("UC"):
                continue
            if title.strip().endswith("- Topic"):
                continue  # chaîne auto-générée sans uploads
            if _uploads_ok(cid, key):
                return cid
        return None
