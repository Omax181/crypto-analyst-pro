...
_CORR_SERIES = {
    ...
    "vix": "VIXCLS",
    "us_10y": "DGS10",
    # v18 (Chantier E) — séries pour les signaux d'analyse transverses :
    "m2": "M2SL",                  # #9 liquidité globale (mensuel)
    "hy_spread": "BAMLH0A0HYM2",   # #11 spread high yield (risk-off avancé)
    # v22 (P2 #33/#34/#35) — taux réels, courbe et liquidité Fed fine :
    "us_2y": "DGS2",               # #34 courbe 2s10s (récession/cycle)
    "real_10y": "DFII10",          # #33 taux réel 10Y (TIPS) = coût d'opportunité
    "fed_assets": "WALCL",         # #35 bilan Fed (hebdo) = QE/QT
    "reverse_repo": "RRPONTSYD",   # #35 reverse repo = liquidité drainée
}
...
_UPCOMING_RELEASES = {
    10: ("Inflation CPI", "high"),
    50: ("Emploi US (NFP + chômage)", "high"),
    21: ("Revenus & dépenses (PCE)", "high"),
    53: ("PIB US (1re estimation)", "high"),
    9: ("Ventes au détail US", "high"),
    # v23.x — ÉLARGISSEMENT (le calendrier hebdo ne montrait souvent qu'UN seul
    # événement) : publications à forte/moyenne sensibilité marché, dont les
    # inscriptions chômage HEBDOMADAIRES → au moins 1 événement garanti chaque
    # semaine, dates RÉELLES FRED (fiable depuis datacenter, contrairement au
    # calendrier Boursorama rendu en JS). IDs FRED /release officiels.
    46: ("Prix à la production (PPI)", "high"),
    180: ("Inscriptions chômage hebdo (jobless claims)", "medium"),
    192: ("Offres d'emploi (JOLTS)", "medium"),
    13: ("Production industrielle US", "medium"),
}
