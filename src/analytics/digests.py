...
def onchain_line(cm: dict[str, Any]) -> str:
    ...
    for sym, d in (cm.get("assets") or {}).items():
        seg = []
        # v23 — MVRV recomposé via prix live / realized price daté (estimation
        # quand l'on-chain est périmé) : préfixe « ~ » + note explicite.
        est = bool(d.get("mvrv_live_estimate"))
        if d.get("mvrv") is not None:
            tilde = "~" if est else ""
            seg.append(f"MVRV {tilde}{d['mvrv']} ({d.get('mvrv_zone', '?')})")
        if d.get("nvt") is not None:
            seg.append(f"NVT {d['nvt']}")
        if d.get("realized_price_ratio") is not None:
            rr = d["realized_price_ratio"]
            tilde = "~" if est else ""
            seg.append(f"prix/realized {tilde}{rr} ({'profit' if rr >= 1 else 'perte'} latent)")
        if d.get("active_addresses_trend_pct") is not None:
            seg.append(f"adresses actives {d['active_addresses_trend_pct']:+}% / 7j")
        if seg and d.get("as_of") and (d.get("stale") or est):
            ymd = str(d["as_of"]).split("-")
            if len(ymd) == 3:
                if est:
                    seg.append(
                        f"MVRV estimé : prix live / prix de revient au "
                        f"{ymd[2]}/{ymd[1]}"
                    )
                else:
