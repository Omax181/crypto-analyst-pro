...
def compute_correlation_analysis(
    ...
    # v19/V18-M3 — CORRECTIF : le dénominateur du « % du portefeuille » doit être
    # la valeur TOTALE du PTF (toutes positions), PAS seulement le sous-ensemble
    # ayant des séries de prix. Sinon un cluster de 10 positions pouvait afficher
    # « 97.6% du portefeuille » alors qu'il ne pèse que ~30% du PTF réel (c'était
    # 97.6% du subset analysé — métrique trompeuse).
...
def _corr_label(key: str, corr: float, n: int | None = None) -> str:
    """Lecture qualitative d'une corrélation BTC ↔ macro (régime de marché).

    v18 (M-A5) : une corrélation BTC↔DXY POSITIVE est structurellement atypique
    (le couple est négativement corrélé la plupart du temps). On l'annote comme
    instable/à confirmer au lieu de la présenter comme un signal propre. Idem si
    l'échantillon est court (< 20 points) : on signale le bruit possible.
    """
    ...
    base = f"{strength} {sign}"
    # DXY positif = inversion du signe habituel → atypique, à confirmer.
    if key == "dxy" and corr > 0.15:
        return f"{base} — atypique (signe habituellement négatif), instable sur fenêtre courte"
    if n is not None and n < 20 and abs(corr) < 0.3:
        return f"{base} — faible significativité (n={n}), à confirmer"
    return base
        ...
        correlations.append(
            {
                ...
                "corr": round(corr, 2),
                "reading": _corr_label(key, corr, n),
            }
        )
